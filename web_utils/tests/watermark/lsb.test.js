const {
  ...
  lsbCapacity,
  LSB_MAGIC,
} = require("../../tools/watermark/lsb.js");
...
describe("lsbCapacity", () => {
  it("calculates capacity for blue channel only", () => {
    // 100x100 = 10000 pixels, 1 bit per pixel, minus 32-bit header, divided by 8, minus LSB_MAGIC (4 bytes)
    expect(lsbCapacity(100, 100, "b")).toBe(
      Math.floor((10000 - 32) / 8) - LSB_MAGIC.length,
    );
  });

  it("calculates capacity for RGB channels", () => {
    // 100x100 = 10000 pixels, 3 bits per pixel = 30000, minus 32, divided by 8, minus LSB_MAGIC (4 bytes)
    expect(lsbCapacity(100, 100, "rgb")).toBe(
      Math.floor((30000 - 32) / 8) - LSB_MAGIC.length,
    );
  });

  it("returns higher capacity for rgb than blue-only", () => {
    expect(lsbCapacity(50, 50, "rgb")).toBeGreaterThan(
      lsbCapacity(50, 50, "b"),
    );
  });
});
