...
describe("_embedBit", () => {
  it("makes |A| > |B| for bit=1", () => {
    ...
    expect(Math.abs(coeffs[_DCT_POS_A])).toBeGreaterThan(
      Math.abs(coeffs[_DCT_POS_B]),
    );
  });
  ...
  it("makes |B| > |A| for bit=0", () => {
    ...
    expect(Math.abs(coeffs[_DCT_POS_B])).toBeGreaterThan(
      Math.abs(coeffs[_DCT_POS_A]),
    );
  });
...
describe("dctCapacity", () => {
  it("calculates blocks and max chars accounting for repetition and magic header", () => {
    ...
    expect(maxChars).toBe(
      Math.floor((logicalBits - 32) / 8) - DCT_MAGIC.length,
    );
  });
...
describe("dctEncode", () => {
  it("throws when message is too long for the canvas", () => {
    ...
    const longMessage =
      "This message is way too long to fit in a tiny image with only 4 blocks";
