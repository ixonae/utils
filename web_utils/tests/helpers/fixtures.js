...
function createMinimalTiff(entries = []) {
  ...
  const entryData = entries.map((e) => {
    ...
    return {
      ...e,
      valueBytes,
      dataOffset: valueBytes.length > 4 ? dataOffset : null,
      _advance:
        valueBytes.length > 4
          ? ((dataOffset += valueBytes.length), true)
          : false,
    };
  });
  ...
  // IFD0
  let pos = 8;
  view.setUint16(pos, entries.length);
  pos += 2;

  for (const e of entryData) {
    view.setUint16(pos, e.tag);
    pos += 2;
    view.setUint16(pos, e.type);
    pos += 2;
    view.setUint32(pos, e.count);
    pos += 4;
