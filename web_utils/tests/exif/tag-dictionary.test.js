const {
  ...
  GPS_TAGS,
  INTEROP_TAGS,
} = require("../../tools/exif/tag-dictionary.js");

describe("lookupTag", () => {
  it("finds IFD0 tags by ID", () => {
    expect(lookupTag(0x010f, "IFD0")).toEqual({
      name: "Make",
      group: "Camera",
    });
    expect(lookupTag(0x0110, "IFD0")).toEqual({
      name: "Model",
      group: "Camera",
    });
  });
  ...
  it("finds IFD1 tags using IFD0 dictionary", () => {
    expect(lookupTag(0x013b, "IFD1")).toEqual({
      name: "Artist",
      group: "Author",
    });
  });
  ...
  it("finds ExifIFD tags", () => {
    expect(lookupTag(0x829a, "ExifIFD")).toEqual({
      name: "ExposureTime",
      group: "Camera",
    });
    expect(lookupTag(0x920a, "ExifIFD")).toEqual({
      name: "FocalLength",
      group: "Camera",
    });
  });
  ...
  it("finds GPS tags", () => {
    expect(lookupTag(0x0002, "GPSIFD")).toEqual({
      name: "GPSLatitude",
      group: "GPS",
    });
  });
  ...
  it("finds Interop tags", () => {
    expect(lookupTag(0x0001, "InteropIFD")).toEqual({
