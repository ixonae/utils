...
const {
    findExifSegment, readSingleOrArray, readRationals,
    formatRational, formatUndefined, parseAllExifTags,
...
describe('formatRational', () => {
    it('formats integer rationals (den=1)', () => {
        expect(formatRational({ num: 100, den: 1, value: 100 }, 'SomeTag')).toBe('100');
    });

    it('formats ExposureTime with 1/x notation', () => {
        expect(formatRational({ num: 1, den: 250, value: 1/250 }, 'ExposureTime')).toBe('1/250s');
    });

    it('formats FNumber with f/ notation', () => {
        expect(formatRational({ num: 28, den: 10, value: 2.8 }, 'FNumber')).toBe('f/2.8');
    });

    it('formats FocalLength with mm notation', () => {
        expect(formatRational({ num: 50, den: 1, value: 50 }, 'FocalLength')).toBe('50');
        expect(formatRational({ num: 185, den: 10, value: 18.5 }, 'FocalLength')).toBe('18.5mm');
    });

    it('formats generic rationals as num/den', () => {
        expect(formatRational({ num: 3, den: 7, value: 3/7 }, 'SomeTag')).toBe('3/7');
    });
});
