...
const _EXIF__EXIF_TYPE_NAMES = ['', 'BYTE', 'ASCII', 'SHORT', 'LONG', 'RATIONAL', 'SBYTE', 'UNDEFINED', 'SSHORT', 'SLONG', 'SRATIONAL', 'FLOAT', 'DOUBLE'];
...
function findExifSegment(jpeg) {
    ...
    while (i < jpeg.length - 1) {
        ...
        if (marker === 0xE1 &&
            jpeg[i + 4] === 0x45 && jpeg[i + 5] === 0x78 &&
            jpeg[i + 6] === 0x69 && jpeg[i + 7] === 0x66 &&
...
function readSingleOrArray(count, stride, getter, valOffset) {
    ...
    return Array.from({ length: count }, (_, i) => getter(valOffset + i * stride));
}
...
function formatUndefined(value, tagName, decoder) {
    ...
    if (tagName === 'UserComment' && value.length >= 8) {
        ...
        return prefix.startsWith('ASCII') ? decoder.decode(value.slice(8)) : `[${value.length} bytes]`;
    }
...
function parseAllExifTags(jpegBytes) {
    ...
    const view = new DataView(jpegBytes.buffer, jpegBytes.byteOffset + tiffOffset);
    ...
    const gs16 = off => bigEndian ? view.getInt16(off) : view.getInt16(off, true);
    const gs32 = off => bigEndian ? view.getInt32(off) : view.getInt32(off, true);
    ...
    function readBytes(valOffset, count) {
        return jpegBytes.slice(tiffOffset + valOffset, tiffOffset + valOffset + count);
    }
    ...
    function readValue(type, count, valOffset) {
        switch (type) {
            case 1:  return readSingleOrArray(count, 1, off => jpegBytes[tiffOffset + off], valOffset); // BYTE
            case 2:  return readAscii(valOffset, count);
            case 3:  return readSingleOrArray(count, 2, g16, valOffset);  // SHORT
            case 4:  return readSingleOrArray(count, 4, g32, valOffset);  // LONG
            case 5:  return readRationals(count, valOffset, g32, g32);     // RATIONAL
            case 6:  return readSignedBytes(valOffset, count);            // SBYTE
            case 7:  return readBytes(valOffset, count);                  // UNDEFINED
            case 8:  return readSingleOrArray(count, 2, gs16, valOffset); // SSHORT
            case 9:  return readSingleOrArray(count, 4, gs32, valOffset); // SLONG
            case 10: return readRationals(count, valOffset, gs32, gs32);   // SRATIONAL
            default: return readBytes(valOffset, count);
        }
    }
    ...
    function formatValue(value, type, tagName) {
        ...
        if (type === 5 || type === 10) {
            ...
            return value.map(r => r.den === 1 ? String(r.num) : `${r.num}/${r.den}`).join(', ');
        }
        if (Array.isArray(value) && value.length > 20) return `[${value.length} values]`;
        if (Array.isArray(value)) return value.join(', ');
        return String(value);
    }

    const SUB_IFD_NAMES = { 0x8769: 'ExifIFD', 0x8825: 'GPSIFD', 0xA005: 'InteropIFD' };
    ...
    function readIFD(offset, ifdName) {
        ...
        let count;
        try { count = g16(offset); } catch { return; }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { findExifSegment, readSingleOrArray, readRationals, formatRational, formatUndefined, parseAllExifTags, _EXIF__EXIF_TYPE_SIZES, _EXIF__EXIF_TYPE_NAMES };
}
