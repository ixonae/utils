...
function stripSelectedTags(jpegBytes, tagKeysToRemove) {
  ...
  function readIFDEntries(offset, ifdName) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (tagId === 0x8769) {
        subIFDName = "ExifIFD";
        subIFD = readIFDEntries(g32(entryOff + 8), "ExifIFD");
      } else if (tagId === 0x8825) {
        subIFDName = "GPSIFD";
        subIFD = readIFDEntries(g32(entryOff + 8), "GPSIFD");
      } else if (tagId === 0xa005) {
        subIFDName = "InteropIFD";
        subIFD = readIFDEntries(g32(entryOff + 8), "InteropIFD");
      }
...
function writeEntryValue(entry, pos, iIdx, eIdx, ctx) {
  ...
  if (entry.isSubIFDPointer) {
    const fixup = subIFDFixups.find(
      (f) => f.parentIfdIdx === iIdx && f.tagId === entry.tagId,
    );
