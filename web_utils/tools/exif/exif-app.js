...
async function handleFile(file) {
    ...
    if (originalBytes[0] !== 0xFF || originalBytes[1] !== 0xD8) {
        showMessage('This file does not appear to be a JPEG. EXIF metadata is primarily found in JPEG files.');
        parsedTags = [];
        renderTagTable();
        updateSummary();
        return;
...
function renderTagTable() {
    ...
    if (parsedTags.length === 0) {
        container.innerHTML = '<div class="placeholder-msg">Upload a JPEG to view its EXIF data</div>';
    ...
    const groupOrder = ['Camera', 'Lens', 'Image', 'Date', 'Author', 'GPS', 'Exif', 'Vendor', 'Interop', 'Other'];
    const sortedGroups = groupOrder.filter(g => groups[g]);
    for (const g of Object.keys(groups)) {
        if (!sortedGroups.includes(g)) sortedGroups.push(g);
    }

    let html = '<table class="exif-table"><thead><tr><th class="exif-col-check"><input type="checkbox" id="selectAll" checked></th><th>Tag</th><th>Value</th><th class="exif-col-id">ID</th></tr></thead><tbody>';

    for (const group of sortedGroups) {
        html += '<tr class="exif-group-row"><td colspan="4">' + escapeHtml(group) + '</td></tr>';

        for (const tag of groups[group]) {
            const key = tag.ifd + ':0x' + tag.tagId.toString(16).padStart(4, '0');
            const displayVal = tag.displayValue.length > 200 ? tag.displayValue.slice(0, 200) + '\u2026' : tag.displayValue;

            html += '<tr data-key="' + escapeHtml(key) + '">' +
                '<td class="exif-col-check"><input type="checkbox" class="tag-checkbox" data-key="' + escapeHtml(key) + '" checked></td>' +
                '<td class="exif-tag-name">' + escapeHtml(tag.name) + '<span class="exif-ifd-badge">' + escapeHtml(tag.ifd) + '</span></td>' +
                '<td class="exif-tag-value" title="' + escapeHtml(tag.displayValue) + '">' + escapeHtml(displayVal) + '</td>' +
                '<td class="exif-col-id exif-tag-id">0x' + tag.tagId.toString(16).padStart(4, '0').toUpperCase() + '</td>' +
                '</tr>';
        }
    }
...
$('btn-strip-selected').addEventListener('click', function () {
    ...
    if (unchecked.size === 0) {
        alert('All tags are selected (checked). Uncheck the tags you want to keep, or use "Strip All".');
        return;
    }
...
function showResultInfo() {
    ...
    resultInfo.innerHTML =
        '<strong>Before:</strong> ' + parsedTags.length + ' tags &rarr; <strong>After:</strong> ' + remainingTags.length + ' tags ' +
        '<span class="result-removed">(' + (parsedTags.length - remainingTags.length) + ' removed)</span>';
