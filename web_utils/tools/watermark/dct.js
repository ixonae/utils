...
function _dctCoeff(block, u, v) {
  ...
  for (let x = 0; x < 8; x++) {
    for (let y = 0; y < 8; y++) {
      sum +=
        block[x * 8 + y] *
        _DCT_COS_TABLE[u * 8 + x] *
        _DCT_COS_TABLE[v * 8 + y];
    }
  }
...
function _bitsToBytes(allBits) {
  ...
  for (
    let i = 0;
    i + _DCT_REPETITIONS - 1 < allBits.length;
    i += _DCT_REPETITIONS
  ) {
    let ones = 0;
  ...
  if (
    length <= 0 ||
    length > _DCT_MAX_LENGTH ||
    length > (decoded.length - 32) / 8
  )
    return null;
...
function dctEncode(canvas, message, strength) {
  ...
  if (bits.length > blocksX * blocksY) {
    throw new Error(
      `Message too long. Need ${bits.length} blocks but only ${blocksX * blocksY} available.`,
    );
  }
...
function dctDecode(canvas) {
  ...
  for (let by = 0; by < blocksY; by++) {
    for (let bx = 0; bx < blocksX; bx++) {
      const coeffs = _dct2d(_readBlock(data, bx, by, w));
      allBits.push(
        Math.abs(coeffs[_DCT_POS_A]) > Math.abs(coeffs[_DCT_POS_B]) ? 1 : 0,
      );
    }
  }
...
function dctCapacity(width, height) {
  ...
  return {
    totalBlocks,
    maxChars: Math.floor((logicalBits - 32) / 8) - DCT_MAGIC.length,
  };
}
