...
function buildExifBytes(fields) {
  ...
  const IFD0_TAGS = new Set([0x013b, 0x8298]); // Artist, Copyright belong in IFD0
  const ifd0Tags = entries.filter((e) => IFD0_TAGS.has(e.tag));
  const exifTags = entries.filter((e) => !IFD0_TAGS.has(e.tag));
  ...
  view.setUint16(pos, 0x4d4d);
  pos += 2;
  view.setUint16(pos, 42);
  pos += 2;
  view.setUint32(pos, 8);
  pos += 4;

  const exifIfdOffset = tiffHeaderSize + ifd0Size;

  view.setUint16(pos, ifd0EntryCount);
  pos += 2;
  ...
  function writeEntry(entry) {
    view.setUint16(pos, entry.tag);
    pos += 2;
    view.setUint16(pos, entry.type);
    pos += 2;
    view.setUint32(pos, entry.count);
    pos += 4;

    if (entry.isExifPointer) {
      view.setUint32(pos, exifIfdOffset);
      pos += 4;
  ...
  if (exifTags.length > 0) {
    ...
    view.setUint16(pos, exifTags.length);
    pos += 2;
...
function _parseWmTiffIFD(data, tiffStart) {
  ...
  function readIFD(offset) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const totalBytes =
        cnt * [0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8][type] || cnt;
      if (totalBytes > 4) valOffset = g32(entryOff + 8);

      if (tag === 0x8769) {
        readIFD(g32(entryOff + 8));
      } else if (tag === 0x013b) {
        result.artist = decoder.decode(
          data.slice(tiffStart + valOffset, tiffStart + valOffset + cnt - 1),
        );
      } else if (tag === 0x8298) {
        result.copyright = decoder.decode(
          data.slice(tiffStart + valOffset, tiffStart + valOffset + cnt - 1),
        );
      } else if (tag === 0x9286) {
        result.userComment = decoder.decode(
          data.slice(tiffStart + valOffset + 8, tiffStart + valOffset + cnt),
        );
      }
    }
  }
