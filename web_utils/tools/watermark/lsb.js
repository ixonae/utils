...
function lsbEncode(canvas, message, channelMode) {
    ...
    if (bits.length > totalCapacity) {
        throw new Error(`Message too long. Need ${bits.length} bits but only ${totalCapacity} available.`);
    }
...
function lsbCapacity(width, height, channelMode) {
    ...
    return Math.floor((pixels * bitsPerPixel - 32) / 8);
}

if (typeof module !== 'undefined' && module.exports) {
    module.exports = { _lsbTextToBits, _lsbBitsToText, lsbEncode, lsbDecode, lsbCapacity, LSB_MAGIC };
}
